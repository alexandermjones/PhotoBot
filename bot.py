    ...
    async def capture_album(self, ctx: commands.Context, album_name: str) -> None:
        ...
        if success:
            if isNew:
                ...
                await ctx.send(f'Users in channel updated. Photos uploaded to this channel will be captured 📷.')
            else:
                await ctx.send(f'Album renamed to: {album_name}. Users in channel updated. Photos uploaded to this channel will be captured 📷.')
        else:
